...
export default function Dashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (p?.length > 0) {
        const latest = p[0]
        setPrices({ acid_oil: latest.acid_oil_price_per_kg, edible_oil: latest.edible_oil_price_per_kg })
        setTempPrices({ acid_oil: latest.acid_oil_price_per_kg, edible_oil: latest.edible_oil_price_per_kg })
      }
      setLoading(false)
    })
  }, [])
  ...
  const excessLossKg = Math.max(0, (actualLoss - theoreticalLoss) / 100 * tpd * 1000)
  const excessLossRs = excessLossKg * prices.edible_oil

  const soapLossKg = (latest?.soap_ppm_post_separator || 650) * tpd * 1000 / 1000000 * 0.68
  const soapLossRs = soapLossKg * prices.edible_oil
  ...
  const separatedRevenue = dailyAcidMT * 1000 * (dfaPrice * 0.635 / prices.acid_oil * parseFloat(prices.acid_oil) + 50 * 0.325)
  ...
  const ChatPanel = () => (
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'white',borderLeft:'1px solid #eee'}}>
      <div style={{padding:'12px 16px',borderBottom:'1px solid #f0f0f0',flexShrink:0,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <button onClick={()=>setMobileChat(false)} className="mobile-only"
          style={{background:'none',border:'1px solid #ddd',padding:'4px 10px',borderRadius:6,cursor:'pointer',fontSize:12,color:'#888',display:'none'}}>
          ← Dashboard
        </button>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'12px',display:'flex',flexDirection:'column',gap:8}}>
        {messages.length === 0 && (
          <div>
            <div style={{textAlign:'center',padding:'12px 0 10px',color:'#aaa',fontSize:12}}>Ask about your process or market</div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {SUGGESTIONS.map((s,i)=>(
                <button key={i} onClick={()=>ask(s)}
                  style={{padding:'8px 12px',background:'#F4F6F8',border:'none',borderRadius:8,cursor:'pointer',fontSize:12,color:'#333',textAlign:'left',lineHeight:1.4}}>
                  {s}
                </button>
              ))}
            </div>
        ...
        {messages.map((m,i)=>(
          <div key={i} style={{display:'flex',flexDirection:'column',alignItems:m.role==='user'?'flex-end':'flex-start'}}>
            {m.role==='user'
              ? <div style={{background:'#1B2A4A',color:'white',padding:'8px 12px',borderRadius:'10px 10px 2px 10px',maxWidth:'85%',fontSize:13,lineHeight:1.5}}>{m.text}</div>
              : <div style={{maxWidth:'95%'}}>
                  <div style={{background:'#F4F6F8',padding:'10px 12px',borderRadius:'2px 10px 10px 10px',fontSize:13,lineHeight:1.7,color:'#222',whiteSpace:'pre-wrap'}}>{m.text}</div>
                  {m.source && SOURCE_LABELS[m.source] && (
                    <span style={{fontSize:10,padding:'2px 6px',borderRadius:20,marginTop:4,display:'inline-block',background:SOURCE_LABELS[m.source].bg,color:SOURCE_LABELS[m.source].color}}>
        ...
        {asking && (
          <div style={{display:'flex',gap:4,padding:'6px 0'}}>
            {[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:'50%',background:'#1D9E75',animation:`bounce 1.2s ${i*0.2}s infinite`}}/>)}
          </div>
        )}
        <div ref={bottomRef}/>
      </div>
      ...
      <div style={{padding:'10px 12px',borderTop:'1px solid #f0f0f0',flexShrink:0}}>
        <div style={{display:'flex',gap:8}}>
          <input value={question} onChange={e=>setQuestion(e.target.value)}
            ...
            style={{flex:1,padding:'8px 12px',fontSize:13,border:'1px solid #ddd',borderRadius:8,outline:'none',color:'#222',background:'white'}}/>
          <button onClick={()=>ask()} disabled={asking||!question.trim()}
            style={{padding:'8px 14px',background:question.trim()?'#1D9E75':'#ccc',color:'white',border:'none',borderRadius:8,fontSize:13,cursor:question.trim()?'pointer':'not-allowed',fontWeight:600}}>
            Ask
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',fontFamily:'sans-serif',background:'#F4F6F8'}}>
      ...
      {/* Header */}
      <div style={{background:'#1B2A4A',padding:'12px 20px',flexShrink:0,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>setMobileChat(c=>!c)} className="mobile-only"
            style={{background:mobileChat?'#1D9E75':'rgba(255,255,255,0.15)',border:'none',color:'white',padding:'5px 12px',borderRadius:6,cursor:'pointer',fontSize:12,display:'none'}}>
            {mobileChat ? '← Dashboard' : '💬 Ask'}
          </button>
          <button onClick={logout} style={{background:'none',border:'1px solid #555',color:'#ccc',padding:'5px 12px',borderRadius:6,cursor:'pointer',fontSize:12}}>Sign out</button>
        </div>
      </div>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',minHeight:0}}>
        ...
        {/* ── LEFT: Dashboard scroll area ── */}
        <div style={{flex:1,overflowY:'auto',padding:'14px 14px 80px',minWidth:0}} className={mobileChat?'hide-mobile':''}>
          ...
          <div style={{background:'white',borderRadius:12,padding:14,boxShadow:'0 1px 6px rgba(0,0,0,0.05)',marginBottom:10}}>
            ...
            {editPrices ? (
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                {[
                  ...
                  ['Distilled FFA','₹'+(parseFloat(prices.acid_oil)+40)+'/kg','#E6F1FB','#042C53'],
            ...
            <div style={{fontSize:10,color:'#aaa',marginTop:8}}>Distilled FFA = Acid oil + ₹40 fixed premium · Update prices daily for accurate financials</div>
          </div>
          ...
          <div style={{background:'white',borderRadius:12,padding:14,boxShadow:'0 1px 6px rgba(0,0,0,0.05)',marginBottom:10}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {[
                ...
                <div key={label} style={{display:'flex',justifyContent:'space-between',padding:'8px 12px',borderRadius:8,background:bg}}>
          ...
          <div style={{background:'white',borderRadius:12,padding:14,boxShadow:'0 1px 6px rgba(0,0,0,0.05)',marginBottom:10}}>
            ...
            <div style={{fontSize:11,color:'#888',marginBottom:10}}>
              {dailyAcidMT.toFixed(1)} MT/day · {avgAcidFFA.toFixed(1)}% FFA avg · Distilled FFA price ₹{dfaPrice}/kg
            </div>
            ...
            <div style={{fontSize:11,color:'#888',lineHeight:1.6}}>
              Two pathways exist to capture this value. Your Kenop team can evaluate which pathway suits your plant's volumes and market access.
            </div>
          </div>
        ...
        {/* ── RIGHT: Chat (desktop always visible) ── */}
        <div style={{width:340,flexShrink:0,overflow:'hidden',display:'flex',flexDirection:'column'}} className="desktop-chat">
          <ChatPanel/>
        </div>

        {/* Mobile chat fullscreen */}
        {mobileChat && (
          <div style={{position:'fixed',top:49,left:0,right:0,bottom:0,zIndex:100,display:'flex',flexDirection:'column'}} className="mobile-chat-overlay">
            <div style={{flex:1,display:'flex',flexDirection:'column',background:'white'}}>
              <div style={{padding:'12px 16px',borderBottom:'1px solid #f0f0f0',flexShrink:0,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <button onClick={()=>setMobileChat(false)}
                  style={{background:'none',border:'1px solid #ddd',padding:'5px 12px',borderRadius:6,cursor:'pointer',fontSize:12,color:'#888'}}>
                  ← Dashboard
                </button>
              </div>
              <div style={{flex:1,overflowY:'auto',padding:'12px',display:'flex',flexDirection:'column',gap:8}}>
                {messages.length===0 && (
                  <div>
                    <div style={{textAlign:'center',padding:'16px 0 12px',color:'#aaa',fontSize:13}}>Ask about your process or market</div>
                    {SUGGESTIONS.map((s,i)=>(
                      <button key={i} onClick={()=>ask(s)}
                        style={{display:'block',width:'100%',padding:'10px 14px',background:'#F4F6F8',border:'none',borderRadius:8,cursor:'pointer',fontSize:13,color:'#333',textAlign:'left',marginBottom:6}}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                {messages.map((m,i)=>(
                  <div key={i} style={{display:'flex',flexDirection:'column',alignItems:m.role==='user'?'flex-end':'flex-start'}}>
                    {m.role==='user'
                      ? <div style={{background:'#1B2A4A',color:'white',padding:'10px 14px',borderRadius:'12px 12px 2px 12px',maxWidth:'85%',fontSize:14,lineHeight:1.5}}>{m.text}</div>
                      : <div style={{maxWidth:'95%'}}>
                          <div style={{background:'#F4F6F8',padding:'12px 14px',borderRadius:'2px 12px 12px 12px',fontSize:14,lineHeight:1.7,color:'#222',whiteSpace:'pre-wrap'}}>{m.text}</div>
                          {m.source&&SOURCE_LABELS[m.source]&&(
                            <span style={{fontSize:11,padding:'2px 8px',borderRadius:20,marginTop:4,display:'inline-block',background:SOURCE_LABELS[m.source].bg,color:SOURCE_LABELS[m.source].color}}>
                              {SOURCE_LABELS[m.source].label}
                            </span>
                          )}
                        </div>
                ...
                {asking&&<div style={{display:'flex',gap:4,padding:'8px 0'}}>{[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:'50%',background:'#1D9E75',animation:`bounce 1.2s ${i*0.2}s infinite`}}/>)}</div>}
                <div ref={bottomRef}/>
              </div>
              <div style={{padding:'12px 16px',borderTop:'1px solid #f0f0f0',flexShrink:0}}>
                <div style={{display:'flex',gap:8}}>
                  <input value={question} onChange={e=>setQuestion(e.target.value)}
                    ...
                    style={{flex:1,padding:'10px 14px',fontSize:14,border:'1px solid #ddd',borderRadius:8,outline:'none',color:'#222',background:'white'}}/>
                  <button onClick={()=>ask()} disabled={asking||!question.trim()}
                    style={{padding:'10px 18px',background:question.trim()?'#1D9E75':'#ccc',color:'white',border:'none',borderRadius:8,fontSize:14,cursor:question.trim()?'pointer':'not-allowed',fontWeight:600}}>
                    Ask
                  </button>
                </div>
              </div>
        ...
        {!mobileChat && (
          <button onClick={()=>setMobileChat(true)} className="mobile-fab"
            style={{position:'fixed',bottom:20,right:20,background:'#1D9E75',color:'white',border:'none',
            borderRadius:'50%',width:54,height:54,fontSize:20,cursor:'pointer',
            boxShadow:'0 4px 12px rgba(29,158,117,0.4)',display:'none',
