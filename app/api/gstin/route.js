...
export async function GET(request) {
  ...
  if (!GSTIN_RE.test(gstin)) {
    return Response.json({ valid: false, error: 'Invalid GSTIN format' })
  }

  const apiKey = process.env.GSTIN_API_KEY
  if (!apiKey) {
    return Response.json({ valid: true, gstin, legalName: '', manualRequired: true })
  }
  ...
  try {
    ...
    // Subscription exhausted or error
    if (d.status === 0) {
      console.error('GSTZen error:', d.message)
      return Response.json({ valid: true, gstin, legalName: '', manualRequired: true })
    }

    // Invalid GSTIN
    if (!d.valid) {
      return Response.json({ valid: false, error: 'GSTIN not found in government records' })
    }
    ...
    console.error('GSTIN lookup failed:', err.message)
    return Response.json({ valid: true, gstin, legalName: '', manualRequired: true,
      error: 'Lookup failed. Enter company name manually.' })
  }
}
